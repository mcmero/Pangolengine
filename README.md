...
The dialogue files read by the engine are in JSON format (see
[assets/scenes/S001_Test.json]) but can be generated by running a Python script
that fetches a Google Drive folder and converts the spreadsheets into JSON
files ready for use by the engine.
