...
- [Objects.tsx](examples/demo/assets/tilesets/Objects.tsx) -- contains sprite objects.
- [TilemapOutdoor.tsx](examples/demo/assets/tilesets/TilemapOutdoor.tsx) -- contains a tile
map.
...
with the name of the map file (which must be present under `assets/maps`
relative to the game code). When a player walks on to the transition layer, the
map will change to the defined map.
...
![Dialogue tree example](examples/demo/assets/docs/dialogue_example.png)
...
`assets/textures` relative to the game code). You can change this per dialogue
line or leave it blank to
...
dialogue file](examples/demo/assets/scenes/S001_Test.json)) but can be generated
by running a Python script that fetches a Google Drive folder and converts the
spreadsheets into JSON files ready for use by the engine.

[The script](examples/demo/scripts/get_dialogue_files.py) uses the Google API
Python Client, so see
...
The script can then be run via (from the `examples/demo` directory):
